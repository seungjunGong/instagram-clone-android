...
class UserThumService (val userThumInterface: UserThumInterface) {
    ...
    fun tryGetUserThumbNaill(getUserId : Int, getPage : Int ){
        val userThumRetrofitInterface = ApplicationClass.sRetrofit.create(UserThumRetrofitInterface::class.java)
        userThumRetrofitInterface.getUserThumbnail(userId = getUserId, page = getPage).enqueue(object : retrofit2.Callback<UserThumResponse> {
